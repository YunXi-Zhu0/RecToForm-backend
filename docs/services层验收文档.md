...
截至本轮重构完成，`src/services/` 已落地以下实现结果：

1. 已新增全局标准字段配置 `template/standard_fields.json`。
2. LLM 已固定输出程序级标准中文 JSON。
3. 模板层已从“识别白名单层”切换为“导出展示方案层”。
4. Workflow 已支持两种导出模式：
   - 模板导出模式
   - 无模板标准字段直出模式
5. Excel 已支持：
   - 按模板映射写入 Excel
...
因此，本文档验收范围在原有基础上增加以下确认点：

- 无模板导出模式不依赖模板资源
- 无模板导出模式仍然消费同一份标准 JSON 契约
- 审计文件可区分 `template` 与 `standard_fields` 两种导出模式
...
4. 当 workflow 不传模板参数时，Excel 可直接按标准字段顺序生成通用表格。
...
- 校验无模板模式下生成的 Excel 第一行是标准字段名、第二行是对应识别值
...
5. 失败时仍保留阶段状态和错误信息。
6. `template_id`、`template_version` 可以为空，并触发无模板标准字段直出分支。
7. 审计文件中能够区分本次导出模式是 `template` 还是 `standard_fields`。
